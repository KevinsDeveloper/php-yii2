...
use lib\vendor\resque\job\DirtyExitException;
use lib\vendor\resque\job\Status;
...
class Worker
{
	...
	public function work($interval = 5)
	{
		...
		while(true) {
			...
			if(!$job) {
				...
				Logger::profile('Sleeping for ' . $interval, __METHOD__);
				if($this->paused) {
			...
			Logger::info('got ' . $job, __METHOD__);
			...
			if ($this->child === 0 || $this->child === false) {
				...
				Logger::profile($status, __METHOD__);
				$this->perform($job);
				if ($this->child === 0) {
					exit(0);
				}
	...
	public function perform(Job $job)
	{
		try {
			...
			Logger::error($job . ' failed: ' . $e->getMessage(), __METHOD__);
			$job->fail($e);
			return;
		}

		$job->updateStatus(Status::STATUS_COMPLETE);
		Logger::info('done ' . $job, __METHOD__);
	}
	...
	public function reserve()
	{
		...
		foreach($queues as $queue) {
			Logger::profile('Checking ' . $queue, __METHOD__);
			$job = Job::reserve($queue);
			if($job) {
				Logger::info('Found job on ' . $queue, __METHOD__);
				return $job;
			}
		}
	...
	private function registerSigHandlers()
	{
		...
		Logger::info('Registered signals', __METHOD__);
	}
	...
	{
		Logger::info('USR2 received; pausing job processing', __METHOD__);
		$this->paused = true;
	}
	...
	{
		Logger::info('CONT received; resuming job processing', __METHOD__);
		$this->paused = false;
	}
	...
	{
		Logger::info('SIGPIPE received; attempting to reconnect', __METHOD__);
	...
	{
		$this->shutdown = true;
		Logger::info('Exiting...', __METHOD__);
	}
	...
	{
		if(!$this->child) {
			Logger::info('No child to kill.', __METHOD__);
			return;
		}

		Logger::info('Killing child at ' . $this->child, __METHOD__);
		if(exec('ps -o pid,state -p ' . $this->child, $output, $returnCode) && $returnCode != 1) {
			...
			Logger::info('Child ' . $this->child . ' not found, restarting.', __METHOD__);
			$this->shutdown();
		}
	}
	...
	public function pruneDeadWorkers()
	{
		...
		foreach($workers as $worker) {
		  if (is_object($worker)) {
			  ...
			  Logger::info('Pruning dead worker: ' . (string)$worker, __METHOD__);
